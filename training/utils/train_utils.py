...
# OmegaConf 是一个强大且灵活的配置管理工具，通常与 Hydra 配置系统一起使用，用于简化和组织配置文件，
# 特别是在机器学习、深度学习和复杂系统中。它支持层次化配置、动态解析、配置的合并与继承，
# 能够让用户通过 YAML 文件或 Python 代码轻松管理和修改配置。
...
def setup_distributed_backend(backend, timeout_mins):
    ...
    # TORCH_NCCL_ASYNC_ERROR_HANDLING 为 "1"，启用 NCCL 异步错误处理。这意味着在分布式训练过程中，
    # 如果有 NCCL（NVIDIA Collective Communications Library）操作发生错误，它将在超时之后主动终止，以避免无限期地等待。
    # NCCL 是 NVIDIA 提供的用于高效 GPU 之间通信的库，通常用于深度学习框架（如 PyTorch）中的多 GPU 通信。
