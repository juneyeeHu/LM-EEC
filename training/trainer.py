...
def sigmoid_focal_loss(
    ...
    return loss.mean(1).sum() / (num_objects*T)
...
class Trainer:
    ...
    def _step(
        self,
        batch: EgoExoBatchedVideoDatapoint,
        model: nn.Module,
        phase: str,
    ):

        outputs = model(batch, self.epoch)

        targets = batch.exo_masks
        batch_size = len(batch.ego_img_batch)

        key = batch.dict_key  # key for dataset
        loss = self.loss[key](outputs, targets)
        valid_affinities = [i["high_res_affinity"] for i in outputs if i["high_res_affinity"] is not None]
        if len(valid_affinities) != 0:
            high_res_affinity_concat = torch.cat(valid_affinities, dim=0)
            loss_affinity = sigmoid_focal_loss(high_res_affinity_concat, targets, 1)
            loss_affinity = loss_affinity*0.0004
            loss['affinity'] = loss_affinity
            loss['core_loss'] = loss['core_loss'] + loss['affinity']
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            dataloader = self.train_dataset.get_loader(epoch=int(self.epoch)) # MixedDataLoader
            barrier()
            start_time = time.time()
            outs = self.train_epoch(dataloader)
            if (self.epoch+1) % 20 == 0:
                torch.save(self.model.state_dict(), self.checkpoint_conf.save_dir + "/" + str(self.epoch)+ ".pth")
    ...
    def train_epoch(self, train_loader):
        ...
        # torch.save(self.model.state_dict(), self.checkpoint_conf.save_dir+".pth")
