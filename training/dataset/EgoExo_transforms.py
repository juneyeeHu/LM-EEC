...
from training.utils.data_utils import VideoDatapoint, EgoExoVideoDatapoint


def hflip(datapoint, index):

    datapoint.ego_frames[index].data = F.hflip(datapoint.ego_frames[index].data)
    datapoint.exo_frames[index].data = F.hflip(datapoint.exo_frames[index].data)
    for obj in datapoint.ego_frames[index].objects:
        if obj.segment is not None:
            obj.segment = F.hflip(obj.segment)
    for obj in datapoint.exo_frames[index].objects:
        if obj.segment is not None:
            obj.segment = F.hflip(obj.segment)
...
def resize(datapoint, index, size, max_size=None, square=False, v2=False):
    ...
    if square:
        ...
        cur_size = (
            datapoint.ego_frames[index].data.size()[-2:][::-1]
            if v2
            else datapoint.ego_frames[index].data.size
        )
    ...
    if v2:
        datapoint.ego_frames[index].data = Fv2.resize(
            datapoint.ego_frames[index].data, size, antialias=True
        )
        datapoint.exo_frames[index].data = Fv2.resize(
            datapoint.exo_frames[index].data, size, antialias=True
        )

    else:
        datapoint.ego_frames[index].data = F.resize(datapoint.ego_frames[index].data, size)
        datapoint.exo_frames[index].data = F.resize(datapoint.exo_frames[index].data, size)
    ...
    h, w = size
    datapoint.ego_frames[index].size = (h, w)
    datapoint.exo_frames[index].size = (h, w)
    return datapoint
    ...
    def __call__(self, datapoint, **kwargs):
        if self.consistent_transform:
            if random.random() < self.p:
                for i in range(len(datapoint.ego_frames)):
...
class ToTensorAPI:
    ...
    def __call__(self, datapoint: EgoExoVideoDatapoint, **kwargs):
        for img in datapoint.ego_frames:
            if self.v2:
                img.data = Fv2.to_image_tensor(img.data)
            else:
                img.data = F.to_tensor(img.data)
        for img in datapoint.exo_frames:
            if self.v2:
                img.data = Fv2.to_image_tensor(img.data)
            else:
                img.data = F.to_tensor(img.data)
...
class RandomAffine:
    ...
    def transform_datapoint(self, datapoint: EgoExoVideoDatapoint):
        _, height, width = F.get_dimensions(datapoint.ego_frames[0].data)
        ...
        for img_idx, img in enumerate(datapoint.ego_frames):
            this_masks = [
        ...
        for img_idx, img in enumerate(datapoint.exo_frames):
            this_masks = [
                obj.segment.unsqueeze(0) if obj.segment is not None else None
                for obj in img.objects
            ]
