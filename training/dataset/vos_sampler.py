...
@dataclass
class EgoExoSampledFramesAndObjects:
    ego_frames: List[int]
    exo_frames: List[int]
    object_ids: List[int]
...
class EgoExoRandomUniformSampler(VOSSampler):
    def __init__(
        self,
        num_frames,
        max_num_objects,
    ...
    def sample(self, ego_video, exo_video, ego_segment_loader, exo_segment_loader, epoch=None):

        for retry in range(MAX_RETRIES):
            if len(ego_video.frames) < self.num_frames:
                raise Exception(
                    f"Cannot sample {self.num_frames} frames from video {ego_video.video_name} as it only has {len(ego_video.frames)} annotated frames."
                )
            start = random.randrange(0, len(ego_video.frames) - self.num_frames + 1)
            ego_frames = [ego_video.frames[start + step] for step in range(self.num_frames)]
            exo_frames = [exo_video.frames[start + step] for step in range(self.num_frames)]
            if random.uniform(0, 1) < self.reverse_time_prob:
                # Reverse time
                ego_frames = ego_frames[::-1]
                exo_frames = exo_frames[::-1]
            # Get first frame object ids
            visible_object_ids = []
            loaded_segms = ego_segment_loader.load(ego_frames[0].frame_idx)
            if isinstance(loaded_segms, LazySegments):
                # LazySegments for SA1BRawDataset
                visible_object_ids = list(loaded_segms.keys())
            else:
                for object_id, segment in ego_segment_loader.load(
                    ego_frames[0].frame_idx
                ).items():
                    if segment.sum():
                        visible_object_ids.append(object_id)
        ...
        return EgoExoSampledFramesAndObjects(ego_frames=ego_frames, exo_frames=exo_frames, object_ids=object_ids)
